...
function Runner ({ bot, log, db, node, name }) {
  ...
  bot._plugins.forEach(plugin => {
    plugin({ bot, node })
  })

  this._setupBotExports()

  const init = bot.runinit({
    userData: this.userData.bind(this),
    sharedData: this.sharedData.bind(this),
    sessionData: this.sessionData.bind(this)
  })

  pump(
    bothandled.createReadStream({ eq: HANDLED_STATUS.todo, live: true, keys: false }),
    through.obj(co(function* (message, enc, cb) {
      yield init
      try {
        var session = yield self.run(message)
      } catch (err) {
...
Runner.prototype.sharedData = function (key) {
  return getOrEmpty(this.dbs.sharedData, key)
}

Runner.prototype.sessionData = function (key) {
  return getOrEmpty(this.dbs.sessionData, key)
}

Runner.prototype.commitSession = co(function* (session) {
  const { userData, sharedData, user } = session
...
Runner.prototype._setupBotExports = function () {
  const self = this
  const bot = this.bot
  Object.keys(bot._exports).forEach(method => {
    const fn = bot._exports[method]
    bot[method] = co(function* (opts) {
      typeforce({
        user: typeforce.String
      }, opts)

      const { user } = opts
      const unlock = yield self.lock(user)
      const results = yield allSettled([
        self.sessionData(user),
        self.userData(user)
      ])

      const [sessionData={}, userData={}] = inspectionValues(results)
      const session = Session.blank({
        runner: self,
        user,
        sessionData,
        userData
      ...
      try {
        ...
        self._debug(`failed in bot export "${method}"`, err)
      } finally {
        unlock()
      }
    })
