...
Runner.prototype.run = co(function* (message) {
  ...
  try {
    ...
    yield Promise.all(session.outbound.map(opts => this.node.send(opts)))
  } catch (err) {
...
Runner.prototype._run = co(function* (message) {
  ...
  const payload = envelope.object
  // message.payloadinfo = yield this.node.objects.get({
  //   link: utils.hexLink(envelope.object),
  //   body: false
  // })

  const session = Session.from({
    runner: this,
    message: normalizeMessage({ message, envelope, payload }),
    userData,
    sessionData
  })
