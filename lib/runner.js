...
function Runner ({ bot, log, db, node, name }) {
  ...
  // used internally
  this.sessionDataDB = promisifiedSubdown(db, 'p', db.options)

  // storage for bot per user
  this.userDataDB = promisifiedSubdown(db, 'u', db.options)

  // general storage for bot
  this.sharedDataDB = promisifiedSubdown(db, 's', db.options)
  ...
  pump(
    ...
    through.obj(function (session, enc, cb) {
      log.append({
        topic: customTopics.bothandled,
        link: session.message.link
      }, cb)
    })
  )
}
...
Runner.prototype.run = co(function* (message) {
  ...
  let session
  try {
    session = yield Promise.race([
      this._run(message),
      promiseTimeout(LOCK_TIMEOUT)
    ])

    yield Promise.all(session.outbound.map(({ message, to }) => {
      return this.node.send({ to, object: message })
    }))
  } catch (err) {
  ...
  const batch = [
    {
      type: 'put',
      key: this.userDataDB.db.prefix + userId,
      value: userData
    }
  ].concat(
    objToBatch(sharedData, this.sharedDataDB.db.prefix)
  )
...
Runner.prototype.userData = function (user) {
  return getOrEmpty(this.userDataDB, user)
}

Runner.prototype.sharedData = function (user) {
  return getOrEmpty(this.sharedDataDB, user)
}

Runner.prototype.sessionData = function (user) {
  return getOrEmpty(this.sessionDataDB, user)
}
...
Runner.prototype._run = co(function* (message) {
  ...
  const session = new Session({
    node: this.node,
    dbs: {
      sharedData: this.sharedDataDB,
      userData: this.userDataDB
    },
    sharedData: {},
    userData: userData,
    userData,
