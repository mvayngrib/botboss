...
exports.normalizeMessage = function ({ message, envelope, payload }) {
  envelope = envelope || message.envelope || message.object
  payload = payload || message.payload || message.object.object
  const payloadMetadata = message.payloadinfo || message.objectinfo
  if (!payloadMetadata.type) {
    payloadMetadata.type = payload[TYPE]
  }
  ...
  return {
    ...
    payload,
    metadata: {
      envelope: pick(message, ['link', 'permalink', 'author']),
...
exports.simpleMsg = function (msg, type) {
  return {
...
exports.calcId = function calcId (metadata) {
  return `${metadata.type}_${metadata.permalink}_${metadata.link}`
}
